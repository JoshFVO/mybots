...
class SOLUTION:
    ...
    def Create_Body(self):
        ...
        for i in range(c.numOfLinks):
            ...
            if i == 0:
                if c.senses[i] == 1:
                    pyrosim.Send_Cube(name="0", pos=[0,0, 0.5] , size=[randSize,randSize,randSize], rgba= '    <color rgba="0 1.0 0 1.0"/>', colorname="Green")
                else:
                ...
                if c.senses[i] == 1:
                    pyrosim.Send_Cube(name="1", pos=[randSize/2,0,0] , size=[randSize,randSize,randSize],  rgba= '    <color rgba="0 1.0 0 1.0"/>', colorname="Green")
                else:
                ...
                if c.senses[i] == 1:
                    pyrosim.Send_Cube(name=str(i), pos=[randSize/2,0,0] , size=[randSize,randSize,randSize], rgba= '    <color rgba="0 1.0 0 1.0"/>', colorname="Green")
                else:
    ...
    def Create_Brain(self):
        ...
        for i in range(c.numOfLinks):
            if c.senses[i] == 1:
                pyrosim.Send_Sensor_Neuron(name = count, linkName= str(i))
                count += 1

        for i in range(c.numOfLinks):
            if i > 0:
                pyrosim.Send_Motor_Neuron(name = count + i  , jointName = str(i-1) + "_" + str(i))
